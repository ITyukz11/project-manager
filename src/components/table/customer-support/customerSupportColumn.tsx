...
export type CustomerSupportForTable = {
  id: string;
  subject: string;
  details: string;
  status: string;
  ...
  customerSupportThreads: { id: string; message: string }[];
};

export const customerSupportColumn: ColumnDef<CustomerSupportForTable>[] = [
  {
    accessorKey: "entryBy",
    header: ({ column }) => (
    ...
    cell: ({ row }) => {
      const customerSupport = row.original;
      // Show Admin's name if present, else NetworkUser name
      const entry = customerSupport.user?.name || "—";
  ...
  {
    accessorKey: "customerSupportThreads",
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title="Threads" />
    ),
    cell: ({ row }) => (
      <span className="font-mono text-xs">
        {row.original.customerSupportThreads?.length ?? 0} msgs
      </span>
    ),
  },
];

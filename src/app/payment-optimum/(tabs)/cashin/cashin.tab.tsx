...
import { getPlatform } from "@/app/droplet/optimum/page";

export type PaymentGatewayMethod = "GCash" | "Chat-Based" | null;
  ...
  const handleProceedToPaymentGateway = async () => {
    ...
    try {
      const payload = {
        ...
        service_type: selectedPayment === "GCash" ? "999" : "???", // map per method
        bank_code: "3021",

        callback_url:
          "https://nxtlink2.xyz/api/optimum-pay/receive-payment-callback",

        hashed_mem_id: externalUserId,
        merchant_user: userName || "guest",
        note: `Deposit via ${selectedPayment}`,
      ...
      const paymentUrl =
        data?.transaction_url || data?.qr_image_url || data?.paymentUrl || null;
            ...
            >
              ...
              {!killSwitchLoading &&
                PAYMENTGATEWAY_METHODS_CASHIN.map((method) => {
                  if (method.id === "QRPH-RAN" || method.id === "Maya")

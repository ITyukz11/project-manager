...
export function CashinFormDialog({
  open,
  onOpenChange,
  onSubmitted,
}: {
  ...
  async function handleSubmit(values: CashinFormValues) {
    setLoading(true);
    try {
      ...
      if (!res.ok) {
        toast.error(data?.error || "Cashin failed!");
        return;
      }

      toast.success("Cashin request submitted successfully!");
      form.reset();
      setPastedImages([]);
      onOpenChange(false);
      refetch();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader>
          <DialogTitle>Request Cashin</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form
              ...
              render={({ field, fieldState }) => (
                <FormItem className="sm:col-span-2">
                  <PasteImageTextarea
                    ...
                    placeholder="Enter Cashin details"

"use client";
import { useCommissionById } from "@/lib/hooks/swr/commission/useCommissionById";
...
export default function Page() {
  ...
  const { commission, isLoading, error, mutate } = useCommissionById(
    id as string
  );
  ...
  async function handleCommentSend(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/commission/${id}/thread`, {
        method: "POST",
        body: formData,
      });
  ...
  //       {!isLoading && commission && commission.commissionThreads.length === 0 && (
  ...
  //           {commission.commissionThreads.map((thread) => {
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-y-4 md:gap-x-8">
          ...
          <div>
            ...
            <Input readOnly value={commission.userName} className="text-sm" />
          </div>
          ...
          <div>
            ...
            <Input
              readOnly
              value={formatAmountWithDecimals(commission.amount)}
          ...
          <div>
            ...
            {/* <Input readOnly value={commission.user?.name} className="text-sm" /> */}
          </div>
            ...
            <Input
              ...
              value={
                commission.createdAt
                  ? formatDate(commission.createdAt, "M/dd/yyyy 'at' hh:mm a")
                  : "—"
              }
              className="text-sm"
            />
          ...
          <div className="md: col-span-1">
            ...
            <div className="border rounded-md p-2 min-h-20 bg-muted/20">
              <ul className="space-y-1">
                {Array.isArray(commission.attachments) &&
                ...
                {Array.isArray(commission.attachments) &&
                  commission.attachments.map((att) => (
          ...
          <div className="flex flex-col sm:flex-row gap-2 md:col-span-2">
            {isAllowed && commission?.status !== "COMPLETED" && (
              <UpdateStatusDialog
                commissionId={id}
                currentStatus={commission?.status}
                externalUserId={commission?.externalUserId}
              />
            )}
            <Button
              variant="outline"
        ...
        <Tabs defaultValue="details" className="w-full">
          <TabsList className="grid w-full grid-cols-2 mb-4">
            ...
            <TabsTrigger value="comments" className="text-sm">
              Comments
              {commission && commission.commissionThreads.length > 0 && (
                <Badge variant="secondary" className="ml-2 text-xs">
                  {commission.commissionThreads.length}
                </Badge>
              )}
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="comments" className="mt-0">
            <div className="px-2 h-[calc(100vh-280px)] flex flex-col">
              <CommentsSection
                threads={commission?.commissionThreads || []}
                isLoading={isLoading}

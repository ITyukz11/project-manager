...
export function UpdateStatusDialog({ customerSupportId, currentStatus }) {
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const { mutate } = useCustomerSupportById(customerSupportId);
  ...
  async function onSubmit(values) {
    ...
    try {
      const res = await fetch(
        `/api/customer-support/${customerSupportId}/status`,
        {
          method: "PATCH",
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Update CustomerSupport Status</DialogTitle>
        </DialogHeader>
